...
(function form() {
  ...
  el.addEventListener('submit', async (e) => {
    ...
    function showDone(sent) {
      ...
      /* Одноразовый только автопереход. Кнопка работает всегда:
         вернувшись со страницы оплаты через «Назад», человек получает
         страницу из кэша браузера вместе с состоянием скрипта —
         одноразовая кнопка осталась бы мёртвой. */
      let auto = true;
      const pay = () => {
        auto = false;
